...
# Production rebalance mode.
#
# rolling_asof_monthly:
#   Anchor the monthly rebalance schedule to the latest available trading day
#   in the downloaded data, then step backward by 1 month, 2 months, 3 months, etc.
#   Each target date maps to the nearest available trading day on or before that
#   target date to avoid lookahead.
#
# Example:
#   If latest close is 2026-07-08, rebalance dates are approximately:
#   2026-07-08, 2026-06-08, 2026-05-08, 2026-04-08, ...
#
# This makes manual and scheduled workflow runs evaluate based on the freshest
# available close rather than stale calendar month-start dates.
REBALANCE_MODE = "rolling_asof_monthly"

# Backwards compatibility. Only used if REBALANCE_MODE == "month_start".
...
def _map_target_to_previous_trading_day(
    target_date: pd.Timestamp,
    trading_index: pd.DatetimeIndex,
) -> pd.Timestamp | None:
    """
    Map a target calendar date to the nearest available trading day on or before it.

    This avoids lookahead. If the target is a weekend/holiday, use the prior
    trading day, not the next future trading day.
    """
...
def get_rebalance_dates(returns_index: pd.DatetimeIndex) -> pd.DatetimeIndex:
    ...
    rolling_asof_monthly:
        Production/live mode. Anchor to the latest available trading day and go
        backward by monthly offsets:
            latest date,
            latest - 1 month,
            latest - 2 months,
            latest - 3 months,
        ...
        Each target date maps to the nearest available trading day on or before
        that target date. Dates are then sorted ascending for backtesting.

        This means if you run the model today with yesterday's close, the whole
        backtest cadence is aligned to yesterday's close, not to calendar
        month-start or month-end.

    month_start:
        first available trading day of each calendar month after LOOKBACK history.

    fixed_days:
        older behavior: every REBALANCE_DAYS trading days after LOOKBACK.
    """
    ...
    rebalance_mode = globals().get("REBALANCE_MODE", "rolling_asof_monthly")

    if rebalance_mode == "fixed_days":
        rebalance_dates = pd.DatetimeIndex(idx[LOOKBACK::REBALANCE_DAYS])

    elif rebalance_mode == "month_start":
        eligible_dates = pd.Series(eligible_idx, index=eligible_idx)
        month_keys = eligible_dates.index.to_period("M")
        rebalance_dates = pd.DatetimeIndex(eligible_dates.groupby(month_keys).first().values)

        if globals().get("INCLUDE_LATEST_ASOF_REBALANCE", False):
            latest_date = pd.Timestamp(idx[-1])
            if len(rebalance_dates) == 0:
                rebalance_dates = pd.DatetimeIndex([latest_date])
            elif latest_date > pd.Timestamp(rebalance_dates[-1]):
                rebalance_dates = pd.DatetimeIndex(list(rebalance_dates) + [latest_date])

    elif rebalance_mode == "rolling_asof_monthly":
        latest_date = pd.Timestamp(idx[-1])
        earliest_allowed = pd.Timestamp(eligible_idx[0])

        mapped_dates: list[pd.Timestamp] = []
        month_offset = 0

        while True:
            target_date = latest_date - pd.DateOffset(months=month_offset)

            if target_date < earliest_allowed:
                break

            mapped = _map_target_to_previous_trading_day(
                target_date=target_date,
                trading_index=idx,
            )

            if mapped is not None and mapped >= earliest_allowed:
                mapped_dates.append(mapped)

            month_offset += 1

            # Safety stop for very long histories.
            if month_offset > 600:
                break

        rebalance_dates = pd.DatetimeIndex(sorted(set(mapped_dates)))

    else:
        raise ValueError(
            f"Unsupported REBALANCE_MODE={rebalance_mode!r}. "
            "Use 'rolling_asof_monthly', 'month_start', or 'fixed_days'."
        )
...
def resolve_end_date_argument(value: str | None) -> str | None:
    """
    Resolve runtime END_DATE override.

    Supported:
      --end-date 2026-07-01
      --end-date auto
      STRATEGY_END_DATE=2026-07-01
      STRATEGY_END_DATE=auto

    yfinance's end date is effectively exclusive, so "auto" uses tomorrow's
    UTC date to include the most recent completed trading day where available.
    """
    ...
    if value.lower() in {"auto", "today", "current"}:
        now_et = datetime.now(ZoneInfo("America/New_York"))

        # Do not allow today's incomplete daily candle into the model.
        # yfinance END_DATE is exclusive:
        #
        # Before 6 PM ET:
        #   END_DATE = today
        #   -> latest included bar is the prior completed trading day
        #
        # At/after 6 PM ET:
        #   END_DATE = tomorrow
        #   -> today's completed daily bar can be included
        safe_market_data_cutoff = now_et.replace(
            hour=18,
            minute=0,
            second=0,
            microsecond=0,
        ...
        if now_et >= safe_market_data_cutoff and now_et.weekday() < 5:
            resolved_date = now_et.date() + timedelta(days=1)
        else:
            resolved_date = now_et.date()

        print(
            f"Auto END_DATE resolved at {now_et:%Y-%m-%d %H:%M:%S %Z}: "
            f"{resolved_date.isoformat()} "
            f"(yfinance exclusive end date)"
        )
    ...
    try:
        ...
        raise ValueError(
            f"Invalid --end-date / STRATEGY_END_DATE value: {value!r}. "
            "Use YYYY-MM-DD or 'auto'."
        ) from exc
...
def parse_runtime_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run V5.16 score-tilted CVaR ETF strategy with optional dynamic END_DATE."
    )

    parser.add_argument(
        "--end-date",
        default=os.environ.get("STRATEGY_END_DATE"),
        help=(
            "Override END_DATE. Use YYYY-MM-DD or 'auto'. "
            "Can also be set with STRATEGY_END_DATE."
        ),
    )

    asof_group = parser.add_mutually_exclusive_group()
    asof_group.add_argument(
        "--include-latest-asof",
        action="store_true",
        default=None,
        help=(
            "Append the latest available trading day as the final live as-of rebalance. "
            "This is the production default in this script."
        ),
    )
    asof_group.add_argument(
        "--no-latest-asof",
        action="store_true",
        default=None,
        help="Disable latest as-of rebalance and use scheduled month-start dates only.",
    )

    parser.add_argument(
        "--rebalance-mode",
        default=os.environ.get("STRATEGY_REBALANCE_MODE"),
        choices=["rolling_asof_monthly", "month_start", "fixed_days"],
        help=(
            "Override rebalance schedule. Production default is rolling_asof_monthly. "
            "Can also be set with STRATEGY_REBALANCE_MODE."
        ),
    )
...
def apply_runtime_overrides(args: argparse.Namespace) -> None:
    ...
    if resolved_end_date is not None:
        ...
        print(f"Runtime override: END_DATE={END_DATE}")
    else:
        print(f"Using script END_DATE={END_DATE}")

    if getattr(args, "rebalance_mode", None):
        REBALANCE_MODE = args.rebalance_mode

    print(f"Runtime rebalance mode: {REBALANCE_MODE}")

    if getattr(args, "include_latest_asof", None) is True:
        INCLUDE_LATEST_ASOF_REBALANCE = True
    elif getattr(args, "no_latest_asof", None) is True:
        INCLUDE_LATEST_ASOF_REBALANCE = False
...
def main() -> None:
    ...
    print(f"Rebalance mode: {globals().get('REBALANCE_MODE', 'rolling_asof_monthly')}")
    print(f"Rolling as-of monthly rebalance mode anchors to latest downloaded close.")
    print(f"Latest as-of final rebalance enabled: {INCLUDE_LATEST_ASOF_REBALANCE}")
