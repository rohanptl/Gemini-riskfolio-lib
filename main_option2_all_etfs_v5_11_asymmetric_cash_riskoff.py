...
MAX_VOL_BASED_CASH_WEIGHT = 1.00
...
# Asymmetric risk-off turnover control.
# Normal ETF-to-ETF rotation remains capped at MAX_TURNOVER_PER_REBALANCE.
# But when the model wants more SGOV/cash, that de-risking move is allowed immediately.
# Moving from SGOV back into risk assets remains controlled by the normal turnover cap.
USE_ASYMMETRIC_CASH_TURNOVER = True
ALLOW_IMMEDIATE_CASH_INCREASE = True
...
OUTPUT_DIR = Path("outputs_option2_v5_11_asymmetric_cash_riskoff")
...
def apply_turnover_cap(
    ...
    """
    Caps turnover with asymmetric treatment for risk-off cash moves.

    V5.11 behavior:
    - If target SGOV/cash weight is increasing, allow that cash increase immediately.
      This lets the strategy de-risk quickly during crashes.
    - The remaining risk sleeve is scaled down immediately to fund that cash increase.
    - ETF-to-ETF rotation inside the remaining risk sleeve is still capped.
    - If target SGOV/cash weight is decreasing, use the old normal turnover cap.
      This makes re-risking gradual instead of instantly chasing a rebound.

    This means reported total turnover may exceed MAX_TURNOVER_PER_REBALANCE
    during risk-off months. That is intentional.
    """
    ...
    # If asymmetric cash handling is disabled or SGOV is not present,
    # use the old symmetric turnover-cap logic.
    if (
        not USE_ASYMMETRIC_CASH_TURNOVER
        or not USE_CASH_SLEEVE
        or CASH_TICKER not in all_assets
    ):
    ...
    prev_cash = float(prev.get(CASH_TICKER, 0.0))
    target_cash = float(target.get(CASH_TICKER, 0.0))

    # ------------------------------------------------------------
    # Fast risk-off path:
    # Cash/SGOV is allowed to increase immediately.
    # ------------------------------------------------------------
    if ALLOW_IMMEDIATE_CASH_INCREASE and target_cash > prev_cash + 1e-12:
        risk_assets = [a for a in all_assets if a != CASH_TICKER]
        desired_risk_total = max(0.0, 1.0 - target_cash)

        final = pd.Series(0.0, index=all_assets, dtype=float)
        final.loc[CASH_TICKER] = target_cash

        if desired_risk_total <= 1e-12:
            final.loc[CASH_TICKER] = 1.0
            return final

        prev_risk = prev.reindex(risk_assets).fillna(0.0).clip(lower=0.0)
        target_risk = target.reindex(risk_assets).fillna(0.0).clip(lower=0.0)

        # Base risk sleeve: previous risk composition scaled down to the new smaller risk budget.
        # This scaling-down funds the cash increase and is not capped.
        if prev_risk.sum() > 1e-12:
            base_risk = prev_risk / prev_risk.sum() * desired_risk_total
        elif target_risk.sum() > 1e-12:
            base_risk = target_risk / target_risk.sum() * desired_risk_total
        else:
            base_risk = pd.Series(0.0, index=risk_assets, dtype=float)

        # Desired risk sleeve: target composition scaled to the remaining risk budget.
        if target_risk.sum() > 1e-12:
            desired_target_risk = target_risk / target_risk.sum() * desired_risk_total
        else:
            desired_target_risk = base_risk.copy()

        # Cap only ETF-to-ETF rotation within the remaining risk sleeve.
        risk_rotation_turnover = 0.5 * np.abs(desired_target_risk - base_risk).sum()

        if risk_rotation_turnover <= max_turnover or risk_rotation_turnover <= 1e-12:
            capped_risk = desired_target_risk
        else:
            blend = max_turnover / risk_rotation_turnover
            capped_risk = base_risk + blend * (desired_target_risk - base_risk)
            capped_risk = capped_risk.clip(lower=0.0)

            if capped_risk.sum() > 0:
                capped_risk = capped_risk / capped_risk.sum() * desired_risk_total

        final.loc[risk_assets] = capped_risk.reindex(risk_assets).fillna(0.0)
    ...
    # ------------------------------------------------------------
    # Risk-on path:
    # If cash is decreasing, keep the old symmetric cap so redeployment is gradual.
    # ------------------------------------------------------------
    raw_turnover = 0.5 * np.abs(target - prev).sum()
...
def run_backtest_for_start_window(
    ...
    print(f"\nRunning V5.11 asymmetric-cash/V5.9b start window {start_date} with {len(rebalance_dates)} rebalance dates...")
...
def main() -> None:
    ...
    print("Cash cap: 100%")
    print("Turnover cap: asymmetric; cash increases are uncapped, ETF rotation remains capped")
