...
MIN_PRINT_WEIGHT = 0.001  # hide tiny residual weights in final printed/saved target

# Eligibility filters.
# V5.4: EMA21 is not a hard universal exclusion.
# It controls whether an ETF can be bought/increased.
# ETFs below EMA21 can remain only as hold/carryover positions through turnover control.
USE_EMA21_BUY_INCREASE_GATE = True
...
OUTPUT_DIR = Path("outputs_option2_v5_4")
...
def compute_asset_score_table(
    ...
    ema_21 = pseudo_prices.ewm(span=min(21, len(pseudo_prices)), adjust=False).mean().iloc[-1]
    ...
    mom_5 = last_price / pseudo_prices.iloc[-min(5, len(pseudo_prices))] - 1
    mom_10 = last_price / pseudo_prices.iloc[-min(10, len(pseudo_prices))] - 1
    ...
    trend_acceleration = mom_21 - mom_63

    vol_63 = r.tail(min(63, len(r))).std() * np.sqrt(TRADING_DAYS)

    above_ema_21 = last_price > ema_21
    above_sma_50 = last_price > sma_50
    above_sma_126 = last_price > sma_126
    positive_mom_5 = mom_5 > 0
    positive_mom_10 = mom_10 > 0
    positive_mom_21 = mom_21 > 0
    ...
    # Score rewards intermediate trend, relative strength, and now gives
    # a smaller boost to short-term trend/acceleration. EMA21 is used mainly
    # as a buy/increase gate, not as a full portfolio exclusion.
    score = (
        1.25 * above_ema_21.astype(float)
        + 2.00 * above_sma_50.astype(float)
        + 3.00 * above_sma_126.astype(float)
        + 0.50 * positive_mom_5.astype(float)
        + 0.75 * positive_mom_10.astype(float)
        + 1.00 * positive_mom_21.astype(float)
        + 2.50 * positive_mom_63.astype(float)
        + 3.50 * positive_mom_126.astype(float)
        + 0.75 * safe_zscore(mom_5)
        + 1.00 * safe_zscore(mom_10)
        + 1.50 * safe_zscore(mom_21)
        + 2.50 * safe_zscore(mom_63)
        + 3.50 * safe_zscore(mom_126)
        + 1.25 * safe_zscore(trend_acceleration)
    ...
    score_table = pd.DataFrame(
        {
            ...
            "SMA50": sma_50,
            "SMA126": sma_126,
            "AboveEMA21": above_ema_21,
            "AboveSMA50": above_sma_50,
            "AboveSMA126": above_sma_126,
            "PositiveMom5": positive_mom_5,
            "PositiveMom10": positive_mom_10,
            "PositiveMom21": positive_mom_21,
    ...
    # Default diagnostics. select_eligible_assets() refines these using
    # the exact active filter settings.
    score_table["BaseEligible"] = False
    score_table["BuyEligible"] = False
    score_table["HoldEligible"] = False
    score_table["CanIncrease"] = False
    score_table["ExitCandidate"] = (
        (~score_table["AboveEMA21"] & ~score_table["AboveSMA50"])
        | ((score_table["Mom21"] < 0) & ~score_table["AboveEMA21"])
    )
...
def select_eligible_assets(train_returns: pd.DataFrame, risk_assets: list[str]) -> tuple[list[str], pd.DataFrame]:
    """
    V5.4 signal/alpha layer:
    1. Build the same V5.2 medium-term winner filter.
    2. Classify ETFs into BuyEligible, HoldEligible, ExitCandidate.
    3. Use EMA21 as a buy/increase gate, not as a full portfolio exclusion.
    4. Prefer BuyEligible ETFs; use HoldEligible only if needed for diversification.
    5. Apply the correlation filter after ranking.
    """
    ...
    # Base eligibility is the original V5.2 medium-term winner filter.
    base_mask = pd.Series(True, index=score_table.index)

    if REQUIRE_PRICE_ABOVE_SMA_50:
        base_mask &= score_table["AboveSMA50"]

    if REQUIRE_PRICE_ABOVE_SMA_126:
        base_mask &= score_table["AboveSMA126"]

    if REQUIRE_POSITIVE_63D_MOMENTUM:
        base_mask &= score_table["PositiveMom63"]

    if REQUIRE_POSITIVE_126D_MOMENTUM:
        base_mask &= score_table["PositiveMom126"]

    if USE_RELATIVE_STRENGTH_FILTER:
        if REQUIRE_OUTPERFORM_SPY_63D:
            base_mask &= score_table["OutperformSPY63"]
        if REQUIRE_OUTPERFORM_SPY_126D:
            base_mask &= score_table["OutperformSPY126"]

    if USE_VOL_FILTER:
        base_mask &= score_table["Vol63Ann"] <= MAX_ELIGIBLE_VOL_63

    # EMA21 controls fresh buying/increasing.
    buy_mask = base_mask.copy()

    if USE_EMA21_BUY_INCREASE_GATE:
        buy_mask &= score_table["AboveEMA21"]

    # HoldEligible means the ETF still passes the medium-term filter,
    # but it is not allowed to be increased if it fails EMA21.
    hold_mask = base_mask & ~buy_mask
    ...
    score_table["BaseEligible"] = base_mask
    score_table["BuyEligible"] = buy_mask
    score_table["HoldEligible"] = hold_mask
    score_table["CanIncrease"] = buy_mask
    score_table["ExitCandidate"] = exit_mask

    ranked_buy = score_table.loc[buy_mask].index.tolist()
    ranked_hold = score_table.loc[hold_mask].index.tolist()

    # Prefer fresh buy/increase candidates.
    if len(ranked_buy) >= MIN_ELIGIBLE_ASSETS:
        eligible = ranked_buy[:MAX_ELIGIBLE_ASSETS]
    else:
        eligible = list(dict.fromkeys(ranked_buy + ranked_hold))[:MAX_ELIGIBLE_ASSETS]

    if len(eligible) < MIN_ELIGIBLE_ASSETS and ALLOW_FALLBACK_TO_TOP_RANKED:
        # Relax relative strength first, but keep trend, momentum, volatility,
        # and EMA21 when possible.
        ...
        fallback_buy_mask = fallback_mask.copy()

        if USE_EMA21_BUY_INCREASE_GATE:
            fallback_buy_mask &= score_table["AboveEMA21"]

        fallback_buy = score_table.loc[fallback_buy_mask].index.tolist()
        fallback_hold = score_table.loc[fallback_mask & ~fallback_buy_mask].index.tolist()

        eligible = list(dict.fromkeys(eligible + fallback_buy + fallback_hold))[:MAX_ELIGIBLE_ASSETS]

    if len(eligible) < MIN_ELIGIBLE_ASSETS and ALLOW_FALLBACK_TO_TOP_RANKED:
        # Last fallback: prefer top-ranked names above EMA21 first.
        if USE_EMA21_BUY_INCREASE_GATE and "AboveEMA21" in score_table.columns:
            ema_pool = score_table.loc[score_table["AboveEMA21"]].index.tolist()
        else:
            ema_pool = []

        ranked_all = list(dict.fromkeys(ema_pool + score_table.index.tolist()))
        eligible = list(dict.fromkeys(eligible + ranked_all))[:MIN_ELIGIBLE_ASSETS]
    ...
    if len(eligible) < min_names_needed:
        # Prefer ETFs that can be increased; then base-eligible holds; then ranked names.
        feasibility_pool = list(
            dict.fromkeys(
                score_table.loc[score_table["CanIncrease"]].index.tolist()
                + score_table.loc[score_table["BaseEligible"]].index.tolist()
                + score_table.index.tolist()
            )
        )

        eligible = apply_correlation_filter_to_ranked_assets(
            train_returns=r,
            ranked_assets=feasibility_pool,
...
def enforce_no_increase_gate(
    ...
    """
    V5.4 guardrail:
    - If an ETF is below EMA21, the optimizer may not increase it above its
      previous rebalance weight.
    - It may remain as a legacy/carryover position because turnover is capped.
    - Excess weight from blocked increases is redirected first to CanIncrease
      ETFs with capacity, then to SGOV/cash if needed.
    """
    if not USE_EMA21_BUY_INCREASE_GATE or score_table is None or score_table.empty:
        return target_weights

    target = target_weights.reindex(all_assets).fillna(0.0).astype(float).clip(lower=0.0)

    if previous_weights is None:
        prev = pd.Series(0.0, index=all_assets)
    else:
        prev = previous_weights.reindex(all_assets).fillna(0.0).astype(float).clip(lower=0.0)

    can_increase = score_table.get("CanIncrease", pd.Series(False, index=score_table.index)).astype(bool)

    blocked_assets = [
        ticker for ticker in target.index
        if ticker != CASH_TICKER
        and ticker in can_increase.index
        and not bool(can_increase.loc[ticker])
    ]

    excess = 0.0

    for ticker in blocked_assets:
        allowed_weight = prev.get(ticker, 0.0)
        if target.loc[ticker] > allowed_weight:
            excess += target.loc[ticker] - allowed_weight
            target.loc[ticker] = allowed_weight
    ...
    allowed_assets = [
        ...
        if ticker != CASH_TICKER
        and ticker in can_increase.index
        and bool(can_increase.loc[ticker])
    ]

    # Add excess to CanIncrease ETFs, respecting the same final-position cap.
    ...
    for _ in range(20):
        ...
        capacity = pd.Series(
            {
                ticker: max(0.0, MAX_RISK_ASSET_WEIGHT - target.loc[ticker])
                for ticker in allowed_assets
            }
        )
        ...
        allocation = remaining_excess * capacity / capacity.sum()
        allocation = np.minimum(allocation, capacity)

        for ticker, add_weight in allocation.items():
            target.loc[ticker] += float(add_weight)

        remaining_excess -= float(allocation.sum())

    # If the buy/increase basket has no remaining capacity, park the rest in SGOV.
    if remaining_excess > 1e-12:
        if USE_CASH_SLEEVE and CASH_TICKER in target.index:
            target.loc[CASH_TICKER] += remaining_excess
        else:
            # Last resort: pro-rata to allowed assets.
            positive_allowed = [ticker for ticker in allowed_assets if target.loc[ticker] > 0]
            if positive_allowed:
                alloc_base = target.loc[positive_allowed]
                alloc_base = alloc_base / alloc_base.sum()
                target.loc[positive_allowed] += remaining_excess * alloc_base
...
def build_final_portfolio_selection_reasons(
    ...
    """
    Human-readable explanation of why each final holding appears.
    Distinguishes fresh buy/increase candidates from residual carryover holdings.
    """
    rows = []
    ...
    for ticker, weight in final_weights.items():
        ...
        if ticker == CASH_TICKER:
            row["Reason"] = "Cash/volatility-control sleeve."
            rows.append(row)
            continue
        ...
        if latest_score_table is not None and not latest_score_table.empty and ticker in latest_score_table.index:
            ...
            row.update(
                {
                    ...
                    "BuyEligible": bool(s.get("BuyEligible", False)),
                    "HoldEligible": bool(s.get("HoldEligible", False)),
                    "CanIncrease": bool(s.get("CanIncrease", False)),
                    "ExitCandidate": bool(s.get("ExitCandidate", False)),
                    "AboveEMA21": bool(s.get("AboveEMA21", False)),
                    "AboveSMA50": bool(s.get("AboveSMA50", False)),
                    "AboveSMA126": bool(s.get("AboveSMA126", False)),
                    "Mom5": s.get("Mom5", np.nan),
                    "Mom10": s.get("Mom10", np.nan),
            ...
            if row["BuyEligible"]:
                row["Reason"] = "Fresh buy/increase candidate: passes medium-term filter and is above EMA21."
            elif row["HoldEligible"]:
                row["Reason"] = "Hold-only candidate: medium-term trend is intact, but EMA21 gate prevents increasing."
            elif row["ExitCandidate"]:
                row["Reason"] = "Exit/decay candidate: weak short-term trend; remains only because turnover is capped."
            elif ticker in latest_eligible_set:
                row["Reason"] = "Included in latest eligible basket by fallback/correlation/cap-feasibility logic."
            else:
                row["Reason"] = "Residual carryover from prior rebalance due to turnover cap."
        else:
            row["Reason"] = "No latest score data available; likely residual carryover or non-risk sleeve."
...
def main() -> None:
    ...
    print(f"\nRunning Option 2 V5.4 EMA21 buy/increase-gated backtest with {len(rebalance_dates)} rebalance dates...")
    ...
    for i, date in enumerate(rebalance_dates):
        ...
        # V5.4: EMA21 is a buy/increase gate.
        # ETFs below EMA21 can remain due to turnover control, but target weights
        # cannot increase them above the previous rebalance weight.
        target_final_weights = enforce_no_increase_gate(
    ...
    print(f"- {OUTPUT_DIR / 'final_portfolio_selection_reasons.csv'}")
