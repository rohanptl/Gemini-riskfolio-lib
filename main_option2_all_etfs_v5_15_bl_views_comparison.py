...
# ------------------------------------------------------------
# V5.15 optimizer/view experiment
# ------------------------------------------------------------
# Keep V5.14 selection/correlation fixed.
# Change only how Riskfolio sizes the selected winner basket.
OPTIMIZER_MODE = "classic_cvar_hist_mu"

SCORE_TILT_STRENGTH = 0.35

OPTIMIZER_VIEW_VARIANTS = [
    {
        "name": "baseline_v514_classic_cvar_hist_mu",
        "optimizer_mode": "classic_cvar_hist_mu",
        "description": "V5.14 baseline: Classic CVaR Sharpe with historical mean returns.",
    },
    {
        "name": "bl_mv_momentum_views",
        "optimizer_mode": "bl_mv_momentum_views",
        "description": "Black-Litterman MV Sharpe using build_momentum_views().",
    },
    {
        "name": "score_tilted_cvar",
        "optimizer_mode": "score_tilted_cvar",
        "description": "Classic CVaR Sharpe, then modestly tilt final weights toward the score rank.",
    },
]

RUN_ALL_OPTIMIZER_VIEW_VARIANTS = True
SINGLE_OPTIMIZER_VIEW_VARIANT_NAME = "baseline_v514_classic_cvar_hist_mu"

# Tracks optimizer fallbacks by variant/window.
OPTIMIZER_FAILURE_COUNTS = {}
...
OUTPUT_DIR = Path("outputs_option2_v5_15_bl_views_comparison")
...
def _record_optimizer_failure(message: str) -> None:
    variant = globals().get("CURRENT_VARIANT_NAME", "unknown")
    OPTIMIZER_FAILURE_COUNTS[variant] = OPTIMIZER_FAILURE_COUNTS.get(variant, 0) + 1
    print(message)


def _finalize_optimizer_weight_frame(
...
def _classic_cvar_hist_optimizer(
    ...
    return _finalize_optimizer_weight_frame(w, available_assets, eligible_assets)
...
def _bl_mv_momentum_views_optimizer(
    r: pd.DataFrame,
    eligible_assets: list[str],
) -> pd.Series:
    """
    Black-Litterman optimizer using the existing build_momentum_views() function.

    This directly tests the critique:
    - score/momentum views become expected-return views,
    - Riskfolio sizes the eligible basket using model='BL'.
    """
    available_assets = r.columns.tolist()
    ...
    if w is None or len(w) == 0:
        raise RuntimeError("Black-Litterman MV Sharpe optimizer returned empty weights.")

    return _finalize_optimizer_weight_frame(w, available_assets, eligible_assets)


def _score_tilted_cvar_optimizer(
    r: pd.DataFrame,
    eligible_assets: list[str],
) -> pd.Series:
    """
    Practical bridge test:
    - Keep Classic CVaR Sharpe as the base optimizer.
    - Then modestly tilt the final weights toward the existing score rank.

    This tests whether the score should influence sizing, not just eligibility,
    without fully abandoning CVaR tail-risk construction.
    """
    base = _classic_cvar_hist_optimizer(r, eligible_assets)

    score_table = compute_asset_score_table(r)
    if score_table.empty:
        return base
    ...
    centered = score_rank - score_rank.mean()
    tilt = (1.0 + SCORE_TILT_STRENGTH * centered).clip(lower=0.70, upper=1.30)
...
def optimize_winner_basket(train_returns: pd.DataFrame, eligible_assets: list[str]) -> pd.Series:
    ...
    V5.15 optimizer modes:
    - classic_cvar_hist_mu: V5.14 baseline
    - bl_mv_momentum_views: Black-Litterman views drive sizing
    - score_tilted_cvar: CVaR sizing plus modest score-rank tilt
    """
    ...
    optimizer_mode = globals().get("OPTIMIZER_MODE", "classic_cvar_hist_mu")

    try:
        if optimizer_mode == "classic_cvar_hist_mu":
            return _classic_cvar_hist_optimizer(r, eligible_assets)

        if optimizer_mode == "bl_mv_momentum_views":
            return _bl_mv_momentum_views_optimizer(r, eligible_assets)

        if optimizer_mode == "score_tilted_cvar":
            return _score_tilted_cvar_optimizer(r, eligible_assets)

        raise ValueError(
            f"Unsupported OPTIMIZER_MODE={optimizer_mode!r}. "
            "Use 'classic_cvar_hist_mu', 'bl_mv_momentum_views', or 'score_tilted_cvar'."
        )

    except Exception as primary_error:
        _record_optimizer_failure(
            f"{optimizer_mode} failed. Falling back to Classic CVaR. Error: {primary_error}"
        )
        ...
        try:
            ...
            _record_optimizer_failure(
                f"Classic CVaR fallback failed. Falling back to Classic MAD. Error: {cvar_error}"
            )
            ...
            try:
                ...
                _record_optimizer_failure(
                    f"Classic MAD fallback failed. Falling back to equal weight. Error: {mad_error}"
                )
                return equal_weight(eligible_assets)
...
def run_backtest_for_start_window(
    ...
    print(f"\nRunning V5.14 fast/slow-correlation month-start start window {start_date} with {len(rebalance_dates)} rebalance dates...")
    ...
    strategy_row["StrategyVersion"] = "V5.15_bl_views_comparison"
    strategy_row["Variant"] = globals().get("CURRENT_VARIANT_NAME", "unknown")
    strategy_row["OptimizerMode"] = globals().get("OPTIMIZER_MODE", "unknown")
    strategy_row["OptimizerFailures"] = OPTIMIZER_FAILURE_COUNTS.get(globals().get("CURRENT_VARIANT_NAME", "unknown"), 0)
    ...
    return {
        ...
        "benchmark_summary": benchmark_summary.reset_index().assign(StartWindow=start_date, Variant=globals().get("CURRENT_VARIANT_NAME", "unknown"), OptimizerMode=globals().get("OPTIMIZER_MODE", "unknown")),
        "rolling_12m_summary": rolling_12m_summary.assign(StartWindow=start_date, Variant=globals().get("CURRENT_VARIANT_NAME", "unknown"), OptimizerMode=globals().get("OPTIMIZER_MODE", "unknown")),
    }
...
def _select_optimizer_view_variants_to_run() -> list[dict]:
    if RUN_ALL_OPTIMIZER_VIEW_VARIANTS:
        return OPTIMIZER_VIEW_VARIANTS

    selected = [
        v for v in OPTIMIZER_VIEW_VARIANTS
        if v["name"] == SINGLE_OPTIMIZER_VIEW_VARIANT_NAME
    ]

    if not selected:
        raise ValueError(
            f"SINGLE_OPTIMIZER_VIEW_VARIANT_NAME={SINGLE_OPTIMIZER_VIEW_VARIANT_NAME!r} not found."
        )

    return selected


def _summarize_optimizer_view_variants(walk_forward_summary: pd.DataFrame) -> pd.DataFrame:
    rows = []

    for variant, group in walk_forward_summary.groupby("Variant"):
        row = {
            "Variant": variant,
            "OptimizerMode": group["OptimizerMode"].iloc[0],
            "Windows": int(group.shape[0]),
            "AvgCAGR": group["CAGR"].mean(),
            "MedianCAGR": group["CAGR"].median(),
            "AvgSharpe": group["Sharpe"].mean(),
            "MedianSharpe": group["Sharpe"].median(),
            "AvgVolatility": group["Annual Volatility"].mean(),
            "WorstMaxDrawdown": group["Max Drawdown"].min(),
            "AvgMaxDrawdown": group["Max Drawdown"].mean(),
            "AvgTurnover": group["AverageTurnover"].mean(),
            "AvgFinalCashWeight": group["FinalCashWeight"].mean(),
            "AvgFinalHoldings": group["FinalHoldings"].mean(),
            "TotalOptimizerFailures": group["OptimizerFailures"].max(),
        }

        for bench in BENCHMARK_TICKERS:
            cagr_col = f"CAGREdgeVs{bench}"
            sharpe_col = f"SharpeEdgeVs{bench}"
            dd_col = f"MaxDDEdgeVs{bench}"

            if cagr_col in group.columns:
                row[f"AvgCAGREdgeVs{bench}"] = group[cagr_col].mean()
                row[f"CAGRWinRateVs{bench}"] = (group[cagr_col] > 0).mean()

            if sharpe_col in group.columns:
                row[f"AvgSharpeEdgeVs{bench}"] = group[sharpe_col].mean()
                row[f"SharpeWinRateVs{bench}"] = (group[sharpe_col] > 0).mean()

            if dd_col in group.columns:
                row[f"AvgMaxDDEdgeVs{bench}"] = group[dd_col].mean()
                row[f"MaxDDWinRateVs{bench}"] = (group[dd_col] > 0).mean()
    ...
    if not result.empty:
        result = result.sort_values(
            ["AvgSharpe", "AvgCAGR", "WorstMaxDrawdown"],
...
def main() -> None:
    ...
    variants_to_run = _select_optimizer_view_variants_to_run()
    ...
    print(f"V5.15 keeps V5.14 correlation mode: {CORRELATION_MODE}; fast={FAST_CORR_LOOKBACK_DAYS}d, slow={SLOW_CORR_LOOKBACK_DAYS}d")
    print(f"Running {len(variants_to_run)} optimizer/view variants:")
    ...
    all_rolling_summaries = []

    for variant in variants_to_run:
        globals()["CURRENT_VARIANT_NAME"] = variant["name"]
        globals()["OPTIMIZER_MODE"] = variant["optimizer_mode"]
        OPTIMIZER_FAILURE_COUNTS[variant["name"]] = 0

        variant_output_dir = OUTPUT_DIR / "variants" / variant["name"]
        variant_walk_forward_dir = variant_output_dir / "walk_forward_windows"
        variant_output_dir.mkdir(parents=True, exist_ok=True)
        variant_walk_forward_dir.mkdir(parents=True, exist_ok=True)

        print("\n" + "=" * 80)
        print(f"Running optimizer/view variant: {variant['name']}")
        ...
        for start_date in start_dates:
            ...
            run_output_dir = variant_walk_forward_dir / year_label
        ...
        walk_forward_summary.to_csv(variant_output_dir / "walk_forward_summary.csv")

        benchmark_by_window = pd.concat(
            [r["benchmark_summary"] for r in run_results],
            ignore_index=True,
        )
        benchmark_by_window.to_csv(
            variant_output_dir / "benchmark_comparison_by_window.csv",
            index=False,
        )
        ...
        rolling_12m_summary_by_window.to_csv(
            variant_output_dir / "rolling_12m_summary_by_window.csv",
            index=False,
        )

        all_variant_summaries.append(walk_forward_summary.reset_index())
        all_benchmark_summaries.append(benchmark_by_window)
        all_rolling_summaries.append(rolling_12m_summary_by_window)

        print(f"\nVariant {variant['name']} walk-forward summary:")
        print(walk_forward_summary)

    combined_walk_forward_summary = pd.concat(
        all_variant_summaries,
        ignore_index=True,
    )
    combined_walk_forward_summary.to_csv(
        OUTPUT_DIR / "optimizer_view_walk_forward_summary.csv",
        index=False,
    )

    combined_benchmark_by_window = pd.concat(
        all_benchmark_summaries,
        ignore_index=True,
    )
    combined_benchmark_by_window.to_csv(
        OUTPUT_DIR / "optimizer_view_benchmark_comparison_by_window.csv",
        index=False,
    )

    combined_rolling_summary = pd.concat(
        all_rolling_summaries,
        ignore_index=True,
    )
    combined_rolling_summary.to_csv(
        OUTPUT_DIR / "optimizer_view_rolling_12m_summary_by_window.csv",
        index=False,
    )

    optimizer_view_average_summary = _summarize_optimizer_view_variants(
        combined_walk_forward_summary
    )
    optimizer_view_average_summary.to_csv(
        OUTPUT_DIR / "optimizer_view_average_summary.csv",
        index=False,
    )

    print("\n" + "=" * 80)
    print("V5.15 optimizer/view average summary:")
    print(optimizer_view_average_summary)

    print("\nFiles written:")
    print(f"- {OUTPUT_DIR / 'optimizer_view_average_summary.csv'}")
    print(f"- {OUTPUT_DIR / 'optimizer_view_walk_forward_summary.csv'}")
    print(f"- {OUTPUT_DIR / 'optimizer_view_benchmark_comparison_by_window.csv'}")
    print(f"- {OUTPUT_DIR / 'optimizer_view_rolling_12m_summary_by_window.csv'}")
    print(f"- {OUTPUT_DIR / 'variants' / '<variant>' / 'walk_forward_summary.csv'}")
    print(f"- {OUTPUT_DIR / 'variants' / '<variant>' / 'walk_forward_windows' / '<year>' / 'final_target_weights.csv'}")
    print(f"- {OUTPUT_DIR / 'variants' / '<variant>' / 'walk_forward_windows' / '<year>' / 'final_portfolio_selection_reasons.csv'}")
