...
# V5.13 correlation experiment:
# baseline = original 126-day rolling correlation.
# fast_slow = max(abs(63-day corr), abs(126-day corr)) to catch sudden crowding.
FAST_CORR_LOOKBACK_DAYS = 63
SLOW_CORR_LOOKBACK_DAYS = 126

# V5.13 scoring/correlation experiment harness.
# This intentionally isolates changes:
# 1. baseline_v510: original score + original correlation
# 2. fast_slow_corr_only: original score + fast/slow correlation
# 3. percentile_score_only: simplified percentile score + original correlation
# 4. fast_slow_corr_plus_percentile_score: both changes together
SCORING_MODE = "original"       # "original" or "percentile"
CORRELATION_MODE = "baseline"   # "baseline" or "fast_slow"

EXPERIMENT_VARIANTS = [
    {
        "name": "baseline_v510",
        "scoring_mode": "original",
        "correlation_mode": "baseline",
        "description": "V5.10/V5.9b baseline: original weighted score + 126-day correlation filter.",
    },
    {
        "name": "fast_slow_corr_only",
        "scoring_mode": "original",
        "correlation_mode": "fast_slow",
        "description": "Original weighted score + effective correlation = max(abs(63d), abs(126d)).",
    },
    {
        "name": "percentile_score_only",
        "scoring_mode": "percentile",
        "correlation_mode": "baseline",
        "description": "Simplified equal-weight percentile scoring + original 126-day correlation filter.",
    },
    {
        "name": "fast_slow_corr_plus_percentile_score",
        "scoring_mode": "percentile",
        "correlation_mode": "fast_slow",
        "description": "Combined experiment: simplified percentile score + fast/slow correlation filter.",
    },
]

# If this is True, each variant runs all WALK_FORWARD_START_DATES.
RUN_ALL_VARIANTS = True

# If you want to debug faster, set RUN_ALL_VARIANTS=False and select one variant name here.
SINGLE_VARIANT_NAME = "baseline_v510"
...
OUTPUT_DIR = Path("outputs_option2_v5_13_variant_comparison")
...
def compute_asset_score_table(
    ...
    """
    Scores each ETF using either:

    SCORING_MODE = "original":
        V5.10/V5.9b hand-weighted trend/momentum/volatility score.

    SCORING_MODE = "percentile":
        V5.13 simplified equal-weight cross-sectional percentile score.
        This reduces coefficient overfitting risk by ranking ETFs instead of
        relying on many hardcoded score multipliers.
    ...
    scoring_mode = globals().get("SCORING_MODE", "original")

    if scoring_mode == "percentile":
        # Equal-weighted cross-sectional percentile ranks.
        # Higher is better for momentum and trend. Lower vol is better.
        mom_21_rank = mom_21.rank(pct=True).fillna(0.5)
        mom_63_rank = mom_63.rank(pct=True).fillna(0.5)
        mom_126_rank = mom_126.rank(pct=True).fillna(0.5)
        low_vol_rank = (-vol_63).rank(pct=True).fillna(0.5)

        # Keep trend simple and transparent.
        trend_rank = (
            0.5 * above_sma_50.astype(float)
            + 0.5 * above_sma_126.astype(float)
        )
        ...
        score_source = "percentile_equal_weight"

    elif scoring_mode == "original":
        # V5.7/V5.10 baseline:
        # Score uses only each ETF's own trend, momentum, and volatility.
        # Benchmark-relative metrics are diagnostics only.
        score = (
        ...
        score_source = "original_weighted"

    else:
        raise ValueError(
            f"Unsupported SCORING_MODE={scoring_mode!r}. "
            "Use 'original' or 'percentile'."
        )
    ...
    score_table = pd.DataFrame(
        {
            ...
            "Mom63": mom_63,
            "Mom126": mom_126,
            "Mom21Rank": mom_21_rank,
            "Mom63Rank": mom_63_rank,
            "Mom126Rank": mom_126_rank,
            "LowVolRank": low_vol_rank,
...
def apply_correlation_filter_to_ranked_assets(
    ...
    CORRELATION_MODE = "baseline":
        Original V5.10 behavior: 126-day absolute correlation.

    CORRELATION_MODE = "fast_slow":
        V5.13 experiment: effective correlation is:
            max(abs(63-day corr), abs(126-day corr))

        This catches sudden short-term correlation spikes while still respecting
        longer-term structural overlap.
    """
    ...
    correlation_mode = globals().get("CORRELATION_MODE", "baseline")
    ...
    if correlation_mode == "baseline":
        ...
        if r_fast.shape[0] >= 30 and r_fast.shape[1] >= 2:
            ...
            fast_corr = fast_corr.reindex(index=slow_corr.index, columns=slow_corr.columns).fillna(0.0)
            corr = pd.DataFrame(
...
def run_backtest_for_start_window(
    ...
    strategy_row["ScoringMode"] = globals().get("SCORING_MODE", "unknown")
    strategy_row["CorrelationMode"] = globals().get("CORRELATION_MODE", "unknown")
    ...
    return {
        ...
        "benchmark_summary": benchmark_summary.reset_index().assign(StartWindow=start_date, Variant=globals().get("CURRENT_VARIANT_NAME", "unknown")),
        "rolling_12m_summary": rolling_12m_summary.assign(StartWindow=start_date, Variant=globals().get("CURRENT_VARIANT_NAME", "unknown")),
    }
...
def _select_variants_to_run() -> list[dict]:
    if RUN_ALL_VARIANTS:
        return EXPERIMENT_VARIANTS

    selected = [
        v for v in EXPERIMENT_VARIANTS
        if v["name"] == SINGLE_VARIANT_NAME
    ]

    if not selected:
        raise ValueError(f"SINGLE_VARIANT_NAME={SINGLE_VARIANT_NAME!r} not found.")

    return selected


def _summarize_variants(walk_forward_summary: pd.DataFrame) -> pd.DataFrame:
    """
    Builds a compact comparison summary by variant.

    This makes it easy to see whether a change improves robustness across
    start windows instead of just winning one backtest.
    """
    rows = []
    ...
    for variant, group in walk_forward_summary.groupby("Variant"):
        row = {
            "Variant": variant,
            "ScoringMode": group["ScoringMode"].iloc[0],
            "CorrelationMode": group["CorrelationMode"].iloc[0],
        ...
        for bench in BENCHMARK_TICKERS:
            ...
            if dd_col in group.columns:
                # MaxDDEdge is strategy DD minus benchmark DD.
                # Higher/less negative is better because drawdowns are negative.
...
def main() -> None:
    ...
    variants_to_run = _select_variants_to_run()
    ...
    print(f"Running {len(variants_to_run)} V5.13 variants:")
    ...
    for variant in variants_to_run:
        ...
        globals()["SCORING_MODE"] = variant["scoring_mode"]
        globals()["CORRELATION_MODE"] = variant["correlation_mode"]
    ...
    # ------------------------------------------------------------
    # Consolidated variant comparison outputs
    # ------------------------------------------------------------
    ...
    combined_walk_forward_summary.to_csv(
        OUTPUT_DIR / "variant_walk_forward_summary.csv",
        index=False,
    )
    ...
    combined_benchmark_by_window.to_csv(
        OUTPUT_DIR / "variant_benchmark_comparison_by_window.csv",
        index=False,
    )
    ...
    combined_rolling_summary.to_csv(
        OUTPUT_DIR / "variant_rolling_12m_summary_by_window.csv",
        index=False,
    )

    variant_average_summary = _summarize_variants(combined_walk_forward_summary)
    variant_average_summary.to_csv(
        OUTPUT_DIR / "variant_average_summary.csv",
        index=False,
    )

    print("\n" + "=" * 80)
    print("V5.13 variant average summary:")
    print(variant_average_summary)

    print("\nFiles written:")
    print(f"- {OUTPUT_DIR / 'variant_average_summary.csv'}")
    print(f"- {OUTPUT_DIR / 'variant_walk_forward_summary.csv'}")
    print(f"- {OUTPUT_DIR / 'variant_benchmark_comparison_by_window.csv'}")
    print(f"- {OUTPUT_DIR / 'variant_rolling_12m_summary_by_window.csv'}")
