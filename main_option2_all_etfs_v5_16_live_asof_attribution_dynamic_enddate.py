...
# Keep the monthly historical rebalance schedule, but append the latest available
# trading day as an extra final "as-of" rebalance. This makes the final allocation
# use the most recent downloaded close, usually yesterday's close when --end-date auto is used.
...
def get_rebalance_dates(returns_index: pd.DatetimeIndex) -> pd.DatetimeIndex:
    ...
    INCLUDE_LATEST_ASOF_REBALANCE:
        If True, append the latest available trading day as an extra final rebalance date.
        This does not change the historical monthly backtest path except at the very end,
        where there are usually no forward returns left. It updates the final allocation
        using the freshest available close.
    """
    ...
    if globals().get("INCLUDE_LATEST_ASOF_REBALANCE", False):
        ...
        # Only append if latest_date is after the last scheduled rebalance and
        # enough lookback history exists. This creates the "as of yesterday close"
        # final target allocation for live production.
...
def run_backtest_for_start_window(
    ...
    strategy_row["StrategyVersion"] = "V5.16_live_asof_score_tilted_cvar_with_attribution"
