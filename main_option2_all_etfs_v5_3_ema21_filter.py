...
# Eligibility filters.
REQUIRE_PRICE_ABOVE_EMA_21 = True
...
OUTPUT_DIR = Path("outputs_option2_v5_3")
...
def compute_asset_score_table(
    ...
    score = (
        2.00 * above_ema_21.astype(float)
...
def select_eligible_assets(train_returns: pd.DataFrame, risk_assets: list[str]) -> tuple[list[str], pd.DataFrame]:
    ...
    if REQUIRE_PRICE_ABOVE_EMA_21:
        mask &= score_table["AboveEMA21"]
    ...
    if len(eligible) < MIN_ELIGIBLE_ASSETS and ALLOW_FALLBACK_TO_TOP_RANKED:
        ...
        if REQUIRE_PRICE_ABOVE_EMA_21:
            fallback_mask &= score_table["AboveEMA21"]
        if REQUIRE_PRICE_ABOVE_SMA_50:
    ...
    if len(eligible) < MIN_ELIGIBLE_ASSETS and ALLOW_FALLBACK_TO_TOP_RANKED:
        # Last fallback: use top-ranked names, but keep the EMA21 rule when possible.
        last_fallback_mask = pd.Series(True, index=score_table.index)

        if REQUIRE_PRICE_ABOVE_EMA_21 and "AboveEMA21" in score_table.columns:
            last_fallback_mask &= score_table["AboveEMA21"]

        eligible = score_table.loc[last_fallback_mask].head(MIN_ELIGIBLE_ASSETS).index.tolist()

        # True last resort: if fewer than MIN_ELIGIBLE_ASSETS are above EMA21,
        # fill from top-ranked names so the optimizer can still run.
        if len(eligible) < MIN_ELIGIBLE_ASSETS:
            for ticker in score_table.index:
    ...
    if len(eligible) < min_names_needed:
        # Preserve the EMA21 hard trend gate when possible even in the cap-feasibility fallback.
        feasibility_pool = score_table.index.tolist()

        if REQUIRE_PRICE_ABOVE_EMA_21 and "AboveEMA21" in score_table.columns:
            ema21_pool = score_table.loc[score_table["AboveEMA21"]].index.tolist()
            if len(ema21_pool) >= min_names_needed:
                feasibility_pool = ema21_pool
...
def compute_signal_breadth(
    ...
    trend_breadth = (
        0.20 * score_table["AboveEMA21"].mean()
        + 0.20 * score_table["AboveSMA50"].mean()
        + 0.20 * score_table["AboveSMA126"].mean()
        + 0.20 * score_table["PositiveMom63"].mean()
        + 0.20 * score_table["PositiveMom126"].mean()
    )
...
def main() -> None:
    ...
    print(f"\nRunning Option 2 winner-filtered backtest with {len(rebalance_dates)} rebalance dates...")
