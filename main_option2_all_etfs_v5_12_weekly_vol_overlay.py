...
# ------------------------------------------------------------
# V5.12 weekly volatility risk overlay
# ------------------------------------------------------------
# Slow layer:
#   Full ETF selection + Classic CVaR Sharpe optimization only happens on month-start.
#
# Fast layer:
#   Weekly risk checks can increase SGOV/cash without waiting for month-start.
#
# Re-entry layer:
#   After an emergency risk-off event, risk exposure fades back in gradually.
USE_WEEKLY_RISK_OVERLAY = True

VOL_FAST_WINDOW = 10
VOL_SLOW_WINDOW = 60
VOL_SPIKE_MULTIPLIER = 1.50

# Portfolio-level spike is based on the currently held risk sleeve.
# Market-level spike is based on MARKET_PROXY, usually SPY.
USE_PORTFOLIO_NATR_TRIGGER = True
USE_MARKET_NATR_TRIGGER = True

# Basket warning weight = share of current risk sleeve whose individual ETF NATR is spiking.
BASKET_VOL_WARNING_THRESHOLD = 0.50

# If volatility spikes but the current basket is unavailable, weak breadth can confirm risk-off.
EMERGENCY_BREADTH_CONFIRMATION = 0.45

# Emergency overlay moves to SGOV fast, but not always 100% cash.
# V5.11 showed that 100% cash was too blunt and re-entered too slowly.
EMERGENCY_CASH_WEIGHT = 0.75

# Weekly fade-in after emergency: 0.25 means 25%, 50%, 75%, 100% risk-budget recovery.
REENTRY_STEP = 0.25

# If True, re-entry waits for breadth confirmation. Default False follows the simpler fade-in rule.
REENTRY_REQUIRES_BREADTH_CONFIRMATION = False
MIN_BREADTH_FOR_REENTRY = 0.50
...
OUTPUT_DIR = Path("outputs_option2_v5_12_weekly_vol_overlay")
...
def extract_ohlc_field_panel(raw: pd.DataFrame, field: str) -> pd.DataFrame:
    """
    Extracts one OHLC field from a yfinance result.

    Handles:
    1. MultiIndex: level 0 = field, level 1 = ticker
    2. MultiIndex: level 0 = ticker, level 1 = field
    3. Flat columns for one ticker
    """
    if raw.empty:
        raise ValueError("yfinance returned empty data.")
    ...
    if isinstance(raw.columns, pd.MultiIndex):
        ...
        if field in level0:
            ...
            raise KeyError(f"No {field} found. Columns: {raw.columns}")
    else:
    ...
    if panel.empty:
        raise ValueError(f"No usable {field} columns after cleaning.")
...
def download_ohlc_panels(
    ...
    """
    Downloads adjusted OHLC data.

    auto_adjust=True makes Close, High, and Low internally consistent for
    volatility/NATR calculations while keeping adjusted Close-like return behavior.
    """
    raw = yf.download(
        tickers,
        start=start_date,
...
def get_weekly_risk_check_dates(returns_index: pd.DatetimeIndex) -> pd.DatetimeIndex:
    """
    Returns the last available trading day of each week after enough LOOKBACK history.

    This is usually Friday. If Friday is a holiday, it uses Thursday or the most
    recent available trading day in that week.
    """
    idx = pd.DatetimeIndex(returns_index).sort_values()
...
def get_v512_event_dates(returns_index: pd.DatetimeIndex) -> tuple[pd.DatetimeIndex, set[pd.Timestamp]]:
    """
    Event dates include:
    - weekly risk-check dates
    - month-start rebalance dates

    Monthly rebalance dates are preserved even if they are not the weekly check date.
    """
    ...
    if USE_WEEKLY_RISK_OVERLAY:
        weekly_dates = pd.DatetimeIndex(get_weekly_risk_check_dates(returns_index))
        event_dates = pd.DatetimeIndex(sorted(set(month_start_dates).union(set(weekly_dates))))
    else:
        event_dates = month_start_dates

    return event_dates, month_start_set
...
def calculate_natr(
    ...
    NATR = ATR / Close.
    It captures intraday range and overnight gaps better than close-to-close volatility.
    """
...
def latest_natr_values(
    high: pd.Series,
    low: pd.Series,
    close: pd.Series,
    fast_window: int = VOL_FAST_WINDOW,
    slow_window: int = VOL_SLOW_WINDOW,
) -> tuple[float, float]:
    """
    Returns latest fast and slow NATR values.
    """
    if len(close.dropna()) < slow_window + 1:
        return np.nan, np.nan

    fast = calculate_natr(high, low, close, fast_window).dropna()
    slow = calculate_natr(high, low, close, slow_window).dropna()
...
def build_weighted_portfolio_ohlc(
    ...
    """
    Builds an approximate synthetic OHLC series for the current risk sleeve.

    Because an ETF portfolio does not have an official intraday high/low, this uses
    normalized weighted OHLC of currently held risk assets. It is not perfect, but it
    is a practical smoke alarm for portfolio-level volatility expansion.
    """
    if weights is None or weights.empty:
        return pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=float)

    risk_weights = weights.drop(labels=[CASH_TICKER], errors="ignore")
    ...
    assets = [
        ...
        if a in high_prices.columns and a in low_prices.columns and a in close_prices.columns
    ]

    if not assets:
        return pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=float)

    # Keep only enough history for NATR. Use calendar rows from available data.
    high = high_prices.loc[:date, assets].tail(lookback_days).dropna(axis=1, how="all")
    low = low_prices.loc[:date, assets].tail(lookback_days).dropna(axis=1, how="all")
    close = close_prices.loc[:date, assets].tail(lookback_days).dropna(axis=1, how="all")

    assets = [a for a in assets if a in high.columns and a in low.columns and a in close.columns]

    if not assets or len(close) < VOL_SLOW_WINDOW + 1:
        return pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=float)

    high = high[assets].ffill().dropna(how="any")
    low = low[assets].ffill().dropna(how="any")
    close = close[assets].ffill().dropna(how="any")
    ...
    if len(close) < VOL_SLOW_WINDOW + 1:
        return pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=float)

    w = risk_weights.reindex(assets).fillna(0.0).astype(float)
    if w.sum() <= 0:
        return pd.Series(dtype=float), pd.Series(dtype=float), pd.Series(dtype=float)

    w = w / w.sum()

    base_close = close.iloc[0].replace(0, np.nan)

    norm_high = high.divide(base_close, axis=1) * 100.0
    norm_low = low.divide(base_close, axis=1) * 100.0
    norm_close = close.divide(base_close, axis=1) * 100.0

    portfolio_high = norm_high.mul(w, axis=1).sum(axis=1)
    portfolio_low = norm_low.mul(w, axis=1).sum(axis=1)
    portfolio_close = norm_close.mul(w, axis=1).sum(axis=1)

    return portfolio_high, portfolio_low, portfolio_close
...
def check_volatility_circuit_breaker(
    ...
    """
    V5.12 emergency risk-off trigger.

    Trigger philosophy:
    - NATR detects fast volatility expansion.
    - Portfolio NATR checks current holdings.
    - Market proxy NATR confirms broad market stress.
    - Basket warning weight tells whether volatility stress is broad across held ETFs.

    Emergency if:
    - portfolio or market volatility is spiking, AND
    - either at least 50% of current risk sleeve is also flashing warning,
      or signal breadth is weak enough to confirm risk-off.
    """
    diagnostics = {
        "PortfolioFastNATR": np.nan,
        "PortfolioSlowNATR": np.nan,
        "PortfolioNATRRatio": np.nan,
        "PortfolioNATRSpike": False,
        "MarketFastNATR": np.nan,
        "MarketSlowNATR": np.nan,
        "MarketNATRRatio": np.nan,
        "MarketNATRSpike": False,
        "BasketWarningWeight": 0.0,
        "EmergencyBreadthConfirm": bool(signal_breadth <= EMERGENCY_BREADTH_CONFIRMATION),
    }

    if current_weights is None or current_weights.empty:
        return False, diagnostics

    lookback_days = max(VOL_SLOW_WINDOW + VOL_FAST_WINDOW + 10, VOL_SLOW_WINDOW + 20)

    # Portfolio-level NATR using current risk sleeve.
    if USE_PORTFOLIO_NATR_TRIGGER:
        p_high, p_low, p_close = build_weighted_portfolio_ohlc(
        ...
        if not p_close.empty:
            p_fast, p_slow = latest_natr_values(p_high, p_low, p_close)
            diagnostics["PortfolioFastNATR"] = p_fast
            diagnostics["PortfolioSlowNATR"] = p_slow

            if pd.notna(p_fast) and pd.notna(p_slow) and p_slow > 0:
                p_ratio = p_fast / p_slow
                diagnostics["PortfolioNATRRatio"] = p_ratio
                diagnostics["PortfolioNATRSpike"] = bool(p_ratio > VOL_SPIKE_MULTIPLIER)

    # Market-level NATR using SPY or MARKET_PROXY.
    if USE_MARKET_NATR_TRIGGER and MARKET_PROXY in high_prices.columns and MARKET_PROXY in low_prices.columns and MARKET_PROXY in close_prices.columns:
        m_high = high_prices.loc[:date, MARKET_PROXY].tail(lookback_days).dropna()
        m_low = low_prices.loc[:date, MARKET_PROXY].tail(lookback_days).dropna()
        m_close = close_prices.loc[:date, MARKET_PROXY].tail(lookback_days).dropna()

        common_index = m_high.index.intersection(m_low.index).intersection(m_close.index)
        m_high = m_high.loc[common_index]
        m_low = m_low.loc[common_index]
        m_close = m_close.loc[common_index]

        if len(m_close) >= VOL_SLOW_WINDOW + 1:
            m_fast, m_slow = latest_natr_values(m_high, m_low, m_close)
            diagnostics["MarketFastNATR"] = m_fast
            diagnostics["MarketSlowNATR"] = m_slow

            if pd.notna(m_fast) and pd.notna(m_slow) and m_slow > 0:
                m_ratio = m_fast / m_slow
                diagnostics["MarketNATRRatio"] = m_ratio
                diagnostics["MarketNATRSpike"] = bool(m_ratio > VOL_SPIKE_MULTIPLIER)

    # Basket-level warning weight among current held risk assets.
    risk_weights = current_weights.drop(labels=[CASH_TICKER], errors="ignore")
    risk_weights = risk_weights[risk_weights > 0]

    if not risk_weights.empty:
        warned_weight = 0.0
        total_checked_weight = 0.0

        for asset, weight in risk_weights.items():
            if asset not in high_prices.columns or asset not in low_prices.columns or asset not in close_prices.columns:
                continue

            a_high = high_prices.loc[:date, asset].tail(lookback_days).dropna()
            a_low = low_prices.loc[:date, asset].tail(lookback_days).dropna()
            a_close = close_prices.loc[:date, asset].tail(lookback_days).dropna()

            common_index = a_high.index.intersection(a_low.index).intersection(a_close.index)
            a_high = a_high.loc[common_index]
            a_low = a_low.loc[common_index]
            a_close = a_close.loc[common_index]

            if len(a_close) < VOL_SLOW_WINDOW + 1:
                continue

            a_fast, a_slow = latest_natr_values(a_high, a_low, a_close)

            if pd.notna(a_fast) and pd.notna(a_slow) and a_slow > 0:
                total_checked_weight += float(weight)
                if a_fast > a_slow * VOL_SPIKE_MULTIPLIER:
                    warned_weight += float(weight)

        if total_checked_weight > 0:
            diagnostics["BasketWarningWeight"] = float(warned_weight / total_checked_weight)

    vol_spike = bool(diagnostics["PortfolioNATRSpike"] or diagnostics["MarketNATRSpike"])
    basket_confirm = diagnostics["BasketWarningWeight"] >= BASKET_VOL_WARNING_THRESHOLD
    breadth_confirm = diagnostics["EmergencyBreadthConfirm"]

    is_emergency = bool(vol_spike and (basket_confirm or breadth_confirm))

    return is_emergency, diagnostics


def move_to_cash_overlay(
    current_weights: pd.Series | None,
    all_assets: list[str],
    emergency_cash_weight: float = EMERGENCY_CASH_WEIGHT,
) -> pd.Series:
    """
    Fast risk-off overlay.

    Raises SGOV/cash immediately while preserving the relative composition of the
    remaining risk sleeve. This does not run Riskfolio and does not rotate ETFs.
    """
    final = pd.Series(0.0, index=all_assets, dtype=float)

    if CASH_TICKER not in final.index:
        return final

    emergency_cash_weight = float(np.clip(emergency_cash_weight, 0.0, 1.0))

    if current_weights is None or current_weights.empty:
        final.loc[CASH_TICKER] = 1.0
        return final

    current = current_weights.reindex(all_assets).fillna(0.0).astype(float).clip(lower=0.0)

    if current.sum() > 0:
        current = current / current.sum()

    risk = current.drop(labels=[CASH_TICKER], errors="ignore")
    risk = risk[risk > 0]

    final.loc[CASH_TICKER] = emergency_cash_weight

    if not risk.empty and emergency_cash_weight < 1.0:
        risk = risk / risk.sum() * (1.0 - emergency_cash_weight)
        final.loc[risk.index] = risk
...
def scale_target_weights_by_reentry(
    normal_target_weights: pd.Series,
    all_assets: list[str],
    reentry_tranche: float,
) -> pd.Series:
    """
    Scales the last normal monthly target by the re-entry tranche.

    Example:
    - normal target = 20% SGOV, 80% risk
    - tranche = 0.25
    - scaled target = 80% SGOV, 20% risk
    """
    target = normal_target_weights.reindex(all_assets).fillna(0.0).astype(float).clip(lower=0.0)

    if target.sum() > 0:
        target = target / target.sum()

    tranche = float(np.clip(reentry_tranche, 0.0, 1.0))

    risk = target.drop(labels=[CASH_TICKER], errors="ignore")
    risk = risk[risk > 0]

    scaled = pd.Series(0.0, index=all_assets, dtype=float)

    if not risk.empty:
        scaled_risk = risk * tranche
        scaled.loc[scaled_risk.index] = scaled_risk

    scaled_cash = 1.0 - scaled.drop(labels=[CASH_TICKER], errors="ignore").sum()

    if CASH_TICKER in scaled.index:
        scaled.loc[CASH_TICKER] = max(0.0, scaled_cash)
...
def run_backtest_for_start_window(
    ...
    """
    Runs the V5.12 strategy for one start window.

    V5.12 structure:
    - Weekly event loop.
    - Fast layer: weekly volatility circuit breaker.
    - Fade-in layer: weekly staged re-entry after emergency.
    - Slow layer: full ETF selection + Classic CVaR Sharpe optimization only on month-start.
    """
    ...
    event_dates, month_start_set = get_v512_event_dates(returns.index)
    ...
    normal_monthly_target_weights = None
    reentry_tranche = 1.0
    ...
    print(
        f"\nRunning V5.12 weekly-vol-overlay start window {start_date} "
    ...
    for i, date in enumerate(event_dates):
        ...
        if current_weights is None:
            current_weights = pd.Series(0.0, index=final_assets, dtype=float)

        previous_weights_for_turnover = current_weights.copy()

        is_emergency, vol_diag = check_volatility_circuit_breaker(
            date=date,
            high_prices=high_prices,
            low_prices=low_prices,
            close_prices=close_prices,
        ...
        # ------------------------------------------------------------
        # 1. FAST LAYER: weekly emergency risk-off overlay
        # ------------------------------------------------------------
        if USE_WEEKLY_RISK_OVERLAY and is_emergency:
            current_weights = move_to_cash_overlay(
                current_weights=current_weights,
                all_assets=final_assets,
                emergency_cash_weight=EMERGENCY_CASH_WEIGHT,
            )

            current_weights = prune_tiny_positions_after_turnover(current_weights)

            reentry_tranche = 0.0
            action = "EmergencyRiskOff"

            eligible_assets = latest_eligible_assets.copy()
            score_table = latest_score_table.copy()

        # ------------------------------------------------------------
        # 2. FADE-IN LAYER: weekly staged re-entry after emergency
        # ------------------------------------------------------------
        elif USE_WEEKLY_RISK_OVERLAY and reentry_tranche < 1.0 and normal_monthly_target_weights is not None:
            can_reenter = True

            if REENTRY_REQUIRES_BREADTH_CONFIRMATION:
                can_reenter = signal_breadth >= MIN_BREADTH_FOR_REENTRY

            if can_reenter:
                reentry_tranche = min(1.0, reentry_tranche + REENTRY_STEP)

                target_reentry_weights = scale_target_weights_by_reentry(
                    normal_target_weights=normal_monthly_target_weights,
                    all_assets=final_assets,
                    reentry_tranche=reentry_tranche,
                )

                # Re-entry does not run Riskfolio and does not rotate ETFs.
                # It simply scales back toward the last monthly target.
                current_weights = target_reentry_weights
                current_weights = prune_tiny_positions_after_turnover(current_weights)
                action = "WeeklyFadeIn"
            else:
                action = "WeeklyHoldCash"

            eligible_assets = latest_eligible_assets.copy()
            score_table = latest_score_table.copy()

        # ------------------------------------------------------------
        # 3. SLOW LAYER: month-start optimizer
        # ------------------------------------------------------------
        elif is_month_start or normal_monthly_target_weights is None:
            ...
            normal_monthly_target_weights = target_final_weights.copy()

            current_weights = apply_turnover_cap(
                previous_weights=previous_weights_for_turnover,
        ...
        # Diagnostics available even on weekly events without optimizer.
        active_sleeve_vol = estimate_active_sleeve_vol(
            train_returns=train_returns,
            risk_weights=current_weights.drop(labels=[CASH_TICKER], errors="ignore"),
        )
        breadth_cash_weight = cash_weight_from_breadth(signal_breadth)
        vol_cash_weight = cash_weight_from_vol(active_sleeve_vol)
        cash_weight = float(current_weights.get(CASH_TICKER, 0.0))
        ...
        turnover = turnover_before_prune

        weights_by_date[date] = current_weights.copy()

        if score_table is not None and not score_table.empty and action == "MonthlyRebalance":
        ...
        row_common = {
            ...
            "ReentryTranche": reentry_tranche,
        ...
        signal_by_date.append(row_common.copy())
        ...
        turnover_rows.append(
            {
                **row_common,
                ...
                "ExtraTurnoverFromPrune": 0.0,
            }
        )

        overlay_rows.append(row_common.copy())
        ...
        if not forward_returns.empty:
            period_portfolio_returns = forward_returns.dot(current_weights.reindex(final_assets).fillna(0.0))
            daily_portfolio_returns.append(period_portfolio_returns)

        if i == 0 or (i + 1) % 20 == 0 or i + 1 == len(event_dates) or action in {"EmergencyRiskOff", "MonthlyRebalance"}:
            top_weights = current_weights.sort_values(ascending=False).head(5).round(4).to_dict()
            print(
                f"{start_date} | {date.date()} | {action} | "
                f"turnover={turnover if not np.isnan(turnover) else 0:.2%} | "
                f"cash={cash_weight:.2%} | "
                f"reentry={reentry_tranche:.2f} | "
    ...
    strategy_row["EmergencyEvents"] = int((overlay_df["Action"] == "EmergencyRiskOff").sum())
    strategy_row["FadeInEvents"] = int((overlay_df["Action"] == "WeeklyFadeIn").sum())
    strategy_row["MonthlyRebalanceEvents"] = int((overlay_df["Action"] == "MonthlyRebalance").sum())
...
def main() -> None:
    ...
    print("V5.12 overlay: weekly NATR circuit breaker + staged re-entry")
