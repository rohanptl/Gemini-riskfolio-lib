...
OUTPUT_DIR = Path("outputs_option2_v5")
...
MIN_FINAL_POSITION_WEIGHT = 0.01
...
def clean_final_portfolio_weights(
    weights: pd.Series,
    max_holdings: int = MAX_FINAL_HOLDINGS,
    min_position_weight: float = MIN_FINAL_POSITION_WEIGHT,
) -> pd.Series:
    """
    Removes small leftover positions caused by turnover blending.

    Keeps:
    - SGOV if present
    - top N non-cash positions
    - positions above minimum weight

    Then renormalizes to 100%.
    """
    w = weights.copy().astype(float)
    w = w.replace([np.inf, -np.inf], np.nan).fillna(0.0)
    w = w.clip(lower=0.0)
    ...
    if CASH_TICKER in w.index:
        cash_weight = w.loc[CASH_TICKER]

    non_cash = w.drop(labels=[CASH_TICKER], errors="ignore")
    non_cash = non_cash[non_cash >= min_position_weight]
    non_cash = non_cash.sort_values(ascending=False).head(max_holdings)

    cleaned = pd.Series(0.0, index=w.index)

    if cash_weight > 0 and CASH_TICKER in cleaned.index:
        cleaned.loc[CASH_TICKER] = cash_weight

    cleaned.loc[non_cash.index] = non_cash
...
def main() -> None:
    ...
    for i, date in enumerate(rebalance_dates):
        ...
        final_weights = clean_final_portfolio_weights(target_final_weights)
